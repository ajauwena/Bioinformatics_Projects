...
path_cod_statistics_bwa = r"C:\Users\ajauw\OneDrive\Documents\m_a\w\education\master's\mbinf\w23\binf_6110\projects\p2\alignment_statistics_txt\cod_statistics_bwa"
...
for txt_file in os.listdir(path_cod_statistics_bwa):
    ...
    with open(os.path.join(path_cod_statistics_bwa, txt_file)) as file:
        ...
        # Append the name and all the obtained statistics to the list "list_cod_statistics_bwa".
        list_cod_statistics_bwa.append([name, reads_mapped, raw_total_sequences, error_rate, average_length, average_quality, alignment_completeness])
        # "list_cod_statistics_bwa" is a list of lists.

# Create a data frame "df_cod_statistics_bwa" from the list "list_cod_statistics_bwa".
df_cod_statistics_bwa = pd.DataFrame(list_cod_statistics_bwa, columns=['File_Name', 'Reads_Mapped', 'Raw_Total_Sequences', 'Error_Rate', 'Average_Length', 'Average_Quality', 'Alignment_Completeness'])
